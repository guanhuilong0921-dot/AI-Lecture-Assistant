...
3. 🚨【致命警告 1】：由于你的输出会被解析为 JSON，所有的 LaTeX 反斜杠必须进行【双重转义】！
   👉 必须写成：\\\\frac、\\\\leq、\\\\sqrt、\\\\int、\\\\partial
   ❌ 绝对不能写成：\\frac、\\leq、\\sqrt、\\int、\\partial
4. 🚨【致命警告 2】：JSON 的字符串值内部【绝对不允许出现真实的换行符（Enter键）】！如果你为了 Markdown 排版需要换行，请必须输入四个反斜杠加n (`\\\\n`) 来代替真实的换行！

【极度重要格式警告】：你是一个无情的 API 接口，你必须、且只能输出一个完整闭合的合法 JSON 字典对象！绝对不要包含任何开头问候语、结尾解释！
请你严格按照以下 JSON 模板输出，绝不能遗漏任何一个字段，最后必须以 }} 结尾：
{{
    "recognition": "在这里填入题目的完整文本识别内容",
    "latex": "在这里填入推导过程的核心 LaTeX 公式代码",
    "analysis": "在这里填入符合【解析深度要求】的解析文本\\\\n请记得用双斜杠n来换行",
    "exercise": "在这里填入变式题组\\\\n请记得用双斜杠n来换行",
    "mindmap": "关键词1,关键词2,关键词3"
}}"""
                                messages[0]["content"].append({"text": prompt_text})
                                try:
